...
class OllamaProvider(BaseProvider):
    """
    Ollama local AI client wrapper (Placeholder).
    """
    ...
    async def chat(self, request: ChatRequest) -> ChatResponse:
        raise ProviderModelNotSupportedError("Ollama provider is not yet implemented.")

    async def stream_chat(
        self,
        request: ChatRequest,
    ) -> AsyncGenerator[StreamChunk, None]:
        """Streaming is not yet implemented for Ollama."""
        raise ProviderModelNotSupportedError("Ollama streaming is not yet implemented.")
